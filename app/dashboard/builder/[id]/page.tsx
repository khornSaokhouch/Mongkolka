...
import {
  ...
  Loader2, Check, ToggleLeft, ToggleRight, RefreshCw, Music2, Smartphone, Monitor
} from "lucide-react"
...
function MapEditor({ data, onChange }: { data: any; onChange: (d: any) => void }) {
  return (
    <div className="space-y-4">
      <Field label="Section Title" value={data.title ?? ""} onChange={v => onChange({ ...data, title: v })} />
      <Field label="Address (For Directions button)" value={data.address ?? ""} onChange={v => onChange({ ...data, address: v })} />
      <Field label="Google Maps Embed URL" value={data.mapUrl ?? ""} onChange={v => onChange({ ...data, mapUrl: v })} />
    </div>
  )
}

// Shared toggles moved to the top
...
function renderEditor(section: TemplateSection, onChange: (data: any) => void, handleUpload: any, isUploading: boolean) {
