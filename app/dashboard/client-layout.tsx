...
import {
  LayoutDashboard, CalendarDays, LayoutGrid, Users,
  BarChart3, Settings, Sparkles, LogOut, Menu, X,
  ChevronRight, Bell, Crown, Plus, QrCode
} from "lucide-react"
...
const navItems = [
  ...
  { href: "/dashboard/templates", icon: LayoutGrid, label: "Templates" },
...
function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const content = (
    <div className="flex flex-col h-full">
      ...
      {/* Upgrade Banner */}
      <div className="mx-3 mt-4 p-3 rounded-xl bg-gradient-to-br from-primary/15 to-accent/15 border border-primary/20">
        <div className="flex items-center gap-2 mb-1">
          <Crown className="w-3.5 h-3.5 text-primary" />
          <span className="text-xs font-semibold text-primary">Free Plan</span>
        </div>
        <p className="text-[11px] text-muted-foreground mb-2">Upgrade for unlimited events &amp; templates</p>
        <Link href="/pricing">
          <Button size="sm" className="w-full h-7 text-xs rounded-lg">Upgrade</Button>
        </Link>
      </div>
        ...
        <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b px-4 md:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/dashboard/create">
              <Button size="sm" className="hidden sm:flex gap-1.5 rounded-lg text-xs h-8">
